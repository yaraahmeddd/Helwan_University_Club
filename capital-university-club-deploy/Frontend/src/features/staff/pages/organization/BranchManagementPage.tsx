...
import { Plus, Search, RefreshCw, MapPin, Pencil, Trash2, Link, Loader2, Eye } from "lucide-react";
...
import { BRANCH_IMPORT_FIELDS } from '@/utils/reportExport/importFieldSchemas';
...
export default function BranchManagementPage() {
    ...
    const toggleBranchSportStatus = async (branchSportId: number, newState: boolean) => {
        const status: BranchSport['status'] = newState ? "active" : "inactive";
        try {
            await api.put(`/branch-sports/${branchSportId}`, { status });
            if (expandedBranchId) {
                setBranchSports((p) => {
                    const row = p[expandedBranchId]?.map((item) =>
                        item.id === branchSportId ? { ...item, status } : item,
                    );
                    return row ? { ...p, [expandedBranchId]: row } : p;
                });
            }
        } catch (err) {
    ...
    const importHandle = useTableImport({
        templateId: "branch-import",
        titleEn: "Branches Import Template",
        titleAr: "قالب استيراد الفروع",
        ...
        importRow: async (row) => {
            try {
                await api.post("/branches", {
                    ...
                    location_ar: row.location_ar?.trim() || undefined,
                    location_en: row.location_en?.trim() || undefined,
