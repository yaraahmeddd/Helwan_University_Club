...
export const localizeDays = (daysStr: string, isRtl: boolean): string => {
    ...
    const days = daysStr.split(/[\s╪î,]+/).filter(Boolean);
    ...
    return localized.join(isRtl ? "╪î " : ", ");
};

// All sports now coming from backend. Mock config removed.

export const STATUS_COLORS: Record<string, { bg: string; text: string; border: string }> = {
    "╪¡╪╢┘ê╪▒": { bg: DS.colors.successLight, text: DS.colors.success, border: DS.colors.success + "40" },
    "╪║┘è╪º╪¿": { bg: DS.colors.errorLight, text: DS.colors.error, border: DS.colors.error + "40" },
    "┘é╪º╪»┘à": { bg: DS.colors.primaryLight, text: DS.colors.primary, border: DS.colors.primary + "40" },
