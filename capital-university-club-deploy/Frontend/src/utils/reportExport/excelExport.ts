...
import type { ReportConfig, ReportSheet } from './types';
...
interface PopulateSheetOptions<T> {
...
function populateWorksheet<T>({
  ...
  const titlePrimary = isAr ? (titleAr ?? config.titleAr) : (titleEn ?? config.titleEn);
  const titleSecondary = isAr ? (titleEn ?? config.titleEn) : (titleAr ?? config.titleAr);
  ...
  styleCell(sheet.getCell(5, 1), {
    value: `${byLabel}: ${exporterName}   |   ${roleLabel}: ${exporterRole}   |   ${dateLabel}: ${exportDate}   |   ${totalLabel}: ${rows.length}`,
    fontSize: 8,
    italic: true,
    bgColor: CLR_META_BG,
...
async function downloadWorkbook(workbook: ExcelJS.Workbook, reportId: string): Promise<void> {
  const buffer = await workbook.xlsx.writeBuffer();
  const blob = new Blob([buffer], {
    type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
  });
  const fileName = `${reportId}-report-${new Date().toISOString().slice(0, 10)}.xlsx`;
...
function createWorksheetShell<T>(
  workbook: ExcelJS.Workbook,
  config: ReportConfig<T>,
  tabName: string,
): ExcelJS.Worksheet {
  ...
  return workbook.addWorksheet(sanitizeSheetName(tabName), {
...
export async function exportToExcel<T>(config: ReportConfig<T>): Promise<void> {
  ...
  const sheet = createWorksheetShell(workbook, config, sheetName);
  ...
  await downloadWorkbook(workbook, config.reportId);
}

/** Multi-sheet Excel export — one worksheet per entry in `sheets`. */
export async function exportToExcelMultiSheet<T>(
  ...
  for (const sheetDef of sheets) {
    let tabName = sanitizeSheetName(isAr ? sheetDef.sheetNameAr : sheetDef.sheetNameEn);
    if (usedTabNames.has(tabName)) {
      let suffix = 2;
      while (usedTabNames.has(`${tabName.slice(0, 28)} ${suffix}`)) suffix += 1;
      tabName = sanitizeSheetName(`${tabName.slice(0, 28)} ${suffix}`);
    }
    usedTabNames.add(tabName);

    const sheet = createWorksheetShell(workbook, config, tabName);
    populateWorksheet({
      workbook,
      sheet,
      config,
      rows: sheetDef.rows,
      titleEn: sheetDef.titleEn,
      titleAr: sheetDef.titleAr,
      logoBuffer,
    });
  }
