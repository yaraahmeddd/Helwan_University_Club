  ...
  useEffect(() => {
    ...
    if (storedUser && storedToken) {
      try {
        ...
        // Auto-refresh user info from the server so stale cached fields (name, role, status, privileges)
        // get replaced with the current truth from the DB.
        fetch("http://localhost:3000/api/auth/me", {
          ...
          .then((data) => {
            ...
            const fresh = { ...parsedUser, ...data.user };
            localStorage.setItem("huc_user", JSON.stringify(fresh));
            setUser(buildUserToken(fresh));
          })
          .catch(() => { /* silent fallback to cached user */ });
      } catch (error) {
