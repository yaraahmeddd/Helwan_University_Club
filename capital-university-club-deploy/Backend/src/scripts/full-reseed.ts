...
async function main() {
  ...
  const mt = (code: string) => memberTypes.find((x) => x.code === code)!;
  ...
  const st = (code: string) => staffTypes.find((x) => x.code === code)!;
  ...
  const plan = (code: string) => plans.find((p) => p.plan_code === code)!;
  ...
  const sports = await sportRepo.save(
    sportData.map((s) => ({
  ...
  const branchSportRepo = AppDataSource.getRepository(BranchSport);
  const branchSportRecords: Partial<BranchSport>[] = [];
  ...
  const fieldOhRepo = AppDataSource.getRepository(FieldOperatingHours);
  ...
  const schedRepo = AppDataSource.getRepository(TeamTrainingSchedule);
  ...
  const uniRepo = AppDataSource.getRepository(UniversityStudentDetail);
  const empRepo = AppDataSource.getRepository(EmployeeDetail);
  const retRepo = AppDataSource.getRepository(RetiredEmployeeDetail);
  const outRepo = AppDataSource.getRepository(OutsiderDetail);
  ...
  const createMember = async (opts: {
    email: string; statusAccount: string; statusMember: string;
    firstEn: string; lastEn: string; firstAr: string; lastAr: string;
    typeCode: string; gender: 'male'|'female'; foreign?: boolean; nationality?: string;
  }) => {
    const acc = await accountRepo.save({
  ...
  const relRepo = AppDataSource.getRepository(MemberRelationship);
  for (let i = 0; i < 6; i++) {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    await bookingRepo.save({
      ...
      price,
      status,
      payment_reference: status !== 'pending_payment' && status !== 'cancelled' ? `BK-${randomBytes(4).toString('hex')}` : null,
    ...
    } as DeepPartialBooking);
  }

  // Payments — various states
  console.log('\n=== Seeding payments ===');
  const paymentStatuses: Array<{ status: string; count: number; method: string; type: string }> = [
  ...
  for (const ps of paymentStatuses) {
    for (let i = 0; i < ps.count; i++) {
      ...
      await paymentRepo.save({
        ...
        transaction_id: ps.status === 'completed' ? `TXN-${randomBytes(6).toString('hex').toUpperCase()}` : undefined,
...
// Loose type aliases to keep the TypeORM .save() typechecker happy without importing full DeepPartial chains.
type DeepPartialField = any;
type DeepPartialTeam = any;
type DeepPartialMember = any;
type DeepPartialTM = any;
type DeepPartialBooking = any;
type DeepPartialPayment = any;
type DeepPartialAnnouncement = any;
