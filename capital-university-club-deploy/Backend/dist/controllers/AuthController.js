...
const PrivilegeCalculationService_1 = __importDefault(require("../services/PrivilegeCalculationService"));
    ...
    async login(req, res) {
        try {
            ...
            if (staff) {
                ...
                try {
                    const privilegeCodes = await PrivilegeCalculationService_1.default.calculateFinalPrivilegeCodes(staff.id);
    ...
    async changeCredentials(req, res) {
        try {
            ...
            if (staff_id) {
                ...
                try {
                    const privilegeCodes = await PrivilegeCalculationService_1.default.calculateFinalPrivilegeCodes(staff_id);
            ...
            await this.auditLogService.createLog({
                userName: finalUserName || account.email,
                role: finalRole || account.role,
    ...
    async getCurrentUser(req, res) {
        try {
            ...
            if (account.role === 'staff' || account.role === 'admin') {
                ...
                if (staff) {
                    ...
                    try {
                        ...
                        const privileges = await PrivilegeCalculationService_1.default.calculateFinalPrivileges(staff.id);
                        userData.privileges = privileges.map((p) => p.code);
                    }
                    catch (privError) {
            ...
            if (account.role === 'team_member') {
                const tm = await data_source_1.AppDataSource.getRepository(TeamMember_1.TeamMember).findOne({
                    where: { account_id: account.id },
    ...
    async updateMyProfile(req, res) {
        try {
            ...
            if (birthdate !== undefined)
                member.birthdate = birthdate ? new Date(birthdate) : null;
            if (photo !== undefined)
                member.photo = photo;
