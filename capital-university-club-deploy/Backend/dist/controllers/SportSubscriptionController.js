 ...
 */
class SportSubscriptionController {
    ...
    static async approveSportSubscription(req, res) {
        try {
            ...
            await SportSubscriptionController.logAction(req, 'Approve', `Approved sport subscription for ${subscription.team_member?.first_name_en} ${subscription.team_member?.last_name_en} to team ${subscription.team?.name_en || subscription.team_id}${notes ? ': ' + notes : ''}`, oldValue, newValue);
            return res.json({
                success: true,
    ...
    static async declineSportSubscription(req, res) {
        try {
            ...
            await SportSubscriptionController.teamMemberTeamRepo.save(subscription);
            const newValue = { ...subscription };
            await SportSubscriptionController.logAction(req, 'Decline', `Declined sport subscription for ${subscription.team_member?.first_name_en} ${subscription.team_member?.last_name_en} to team ${subscription.team?.name_en || subscription.team_id}. Reason: ${reason}`, oldValue, newValue);
            return res.json({
                success: true,
    ...
    static async cancelSportSubscription(req, res) {
        try {
            ...
            await SportSubscriptionController.teamMemberTeamRepo.save(subscription);
            const newValue = { ...subscription };
            await SportSubscriptionController.logAction(req, 'Cancel', `Cancelled sport subscription for ${subscription.team_member?.first_name_en} ${subscription.team_member?.last_name_en} to team ${subscription.team?.name_en || subscription.team_id}${reason ? '. Reason: ' + reason : ''}`, oldValue, newValue);
            return res.json({
                success: true,
...
SportSubscriptionController.teamMemberTeamRepo = data_source_1.AppDataSource.getRepository(TeamMemberTeam_1.TeamMemberTeam);
SportSubscriptionController.teamMemberRepo = data_source_1.AppDataSource.getRepository(TeamMember_1.TeamMember);
SportSubscriptionController.staffRepo = data_source_1.AppDataSource.getRepository(Staff_1.Staff);
//# sourceMappingURL=SportSubscriptionController.js.map