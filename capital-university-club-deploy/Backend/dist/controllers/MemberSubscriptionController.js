...
class MemberSubscriptionController {
     ...
     */
    static async getMemberSubscriptions(req, res) {
        try {
     ...
     */
    static async approveSportSubscription(req, res) {
        try {
            const { subscriptionId } = req.params;
            ...
            await MemberSubscriptionController.logAction(req, 'Approve', `Approved sport subscription for ${subscription.member?.first_name_en} ${subscription.member?.last_name_en} to team ${subscription.team?.name_en}${notes ? ': ' + notes : ''}`, oldValue, newValue);
            return res.json({
                success: true,
     ...
     */
    static async declineSportSubscription(req, res) {
        try {
            const { subscriptionId } = req.params;
            ...
            await MemberSubscriptionController.memberTeamRepo.save(subscription);
            const newValue = { ...subscription };
            await MemberSubscriptionController.logAction(req, 'Decline', `Declined sport subscription for ${subscription.member?.first_name_en} ${subscription.member?.last_name_en} to team ${subscription.team?.name_en}. Reason: ${reason}`, oldValue, newValue);
            return res.json({
                success: true,
     ...
     */
    static async cancelSportSubscription(req, res) {
        try {
            const { subscriptionId } = req.params;
            const { reason } = req.body;
            const subscription = await MemberSubscriptionController.memberTeamRepo.findOne({
            ...
            await MemberSubscriptionController.memberTeamRepo.save(subscription);
            const newValue = { ...subscription };
            await MemberSubscriptionController.logAction(req, 'Cancel', `Cancelled sport subscription for ${subscription.member?.first_name_en} ${subscription.member?.last_name_en} to team ${subscription.team?.name_en}${reason ? '. Reason: ' + reason : ''}`, oldValue, newValue);
            return res.json({
                success: true,
     ...
     */
    static async subscribeToTeam(req, res) {
        try {
        ...
        try {
            ...
            const payment = await paymentService.confirmPayment(finalReference, {
                ...
                if (err.message.includes('Payment not found')) {
                    const newPayment = await data_source_1.AppDataSource.getRepository('Payment').save({
...
MemberSubscriptionController.memberTeamRepo = data_source_1.AppDataSource.getRepository(MemberTeam_1.MemberTeam);
MemberSubscriptionController.memberRepo = data_source_1.AppDataSource.getRepository(Member_1.Member);
MemberSubscriptionController.teamRepo = data_source_1.AppDataSource.getRepository(Team_1.Team);
MemberSubscriptionController.staffRepo = data_source_1.AppDataSource.getRepository(Staff_1.Staff);
//# sourceMappingURL=MemberSubscriptionController.js.map