...
exports.MemberBookingController = void 0;
...
class MemberBookingController {
    constructor() {
         ...
         */
        this.getBookableFieldsBySport = async (req, res) => {
            try {
         ...
         */
        this.getFieldCalendar = async (req, res) => {
            try {
        ...
        this.getAvailableSlots = async (req, res) => {
            try {
                ...
                const bookingDate = date ? date : new Date().toISOString().split("T")[0];
         ...
         */
        this.createMemberBooking = async (req, res) => {
            try {
                const { memberId, sport_id, field_id, start_time, end_time, notes } = req.body;
         ...
         */
        this.getMemberBookings = async (req, res) => {
            try {
                ...
                const filteredBookings = status
                    ...
                    : bookings;
                const bookingDetails = await Promise.all(filteredBookings.map((b) => this.bookingService.getBookingDetails(b.id, baseUrl)));
                res.json({
                    success: true,
                    data: bookingDetails,
        ...
        this.getUpcomingBookings = async (req, res) => {
            try {
                ...
                const bookingDetails = await Promise.all(upcomingBookings.map((b) => this.bookingService.getBookingDetails(b.id, baseUrl)));
                res.json({
                    success: true,
                    data: bookingDetails,
                    count: bookingDetails.length,
         ...
         */
        this.getBookingHistory = async (req, res) => {
            try {
                const { memberId } = req.params;
                if (!memberId || isNaN(Number(memberId))) {
                    res.status(400).json({
                ...
                const pastBookings = bookings.filter((b) => new Date(b.start_time) <= now ||
                    b.status === "completed" ||
                    b.status === "cancelled");
                const bookingDetails = await Promise.all(pastBookings.map((b) => this.bookingService.getBookingDetails(b.id, baseUrl)));
                res.json({
                    success: true,
                    data: bookingDetails,
                    count: bookingDetails.length,
         ...
         */
        this.getMemberStats = async (req, res) => {
            try {
                const { memberId } = req.params;
                if (!memberId || isNaN(Number(memberId))) {
                    res.status(400).json({
        ...
        this.fieldService = new FieldService_1.FieldService();
    }
}
exports.MemberBookingController = MemberBookingController;
//# sourceMappingURL=MemberBookingController.js.map