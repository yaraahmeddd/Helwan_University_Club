...
class ProfessionController {
    ...
    static async createProfession(req, res) {
        try {
            ...
            const newProfession = new Profession_1.Profession();
     ...
     */
    static async updateProfession(req, res) {
        try {
            const { id } = req.params;
            const { code, name_en, name_ar } = req.body;
            // Validate ID parameter
     ...
     */
    static async deleteProfession(req, res) {
        try {
            const { id } = req.params;
            // Validate ID parameter
            const professionId = parseInt(id);
     ...
     */
    static async assignProfessionToMember(req, res) {
        try {
            ...
            const employeeDetail = await data_source_1.AppDataSource.getRepository('EmployeeDetail').findOne({
                where: { member_id: mId },
            });
            if (!employeeDetail) {
                return res.status(404).json({
            ...
            await data_source_1.AppDataSource.getRepository('EmployeeDetail').save(employeeDetail);
...
ProfessionController.professionRepo = data_source_1.AppDataSource.getRepository(Profession_1.Profession);
