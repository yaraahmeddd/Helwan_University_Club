...
exports.ParticipantRegistrationController = void 0;
...
class ParticipantRegistrationController {
     ...
     */
    async getBookingByShareToken(req, res) {
        try {
            ...
            const participantCount = await data_source_1.AppDataSource.getRepository("BookingParticipant").count({
                where: { booking_id: booking.id }
            });
            // Get field and sport details
            const field = await data_source_1.AppDataSource.getRepository("Field").findOne({
                where: { id: booking.field_id }
            });
            const sport = await data_source_1.AppDataSource.getRepository("Sport").findOne({
                where: { id: booking.sport_id },
                select: ["id", "name_ar", "name_en"]
            });
            // Prefer Arabic field name, then English, then any generic name
            const fieldName = (field && (field.name_ar || field.name_en || field.name)) ||
                "Unknown Field";
            res.status(200).json({
                success: true,
                data: {
    ...
    async registerParticipant(req, res) {
        try {
            ...
            let national_id_front_url;
            let national_id_back_url;
            try {
                // Upload front photo if provided
                if (files?.national_id_front?.[0]) {
                    const frontFile = files.national_id_front[0];
                    national_id_front_url = await (0, localFileStorage_1.saveToLocalStorage)(frontFile.buffer, frontFile.originalname, localFileStorage_1.DocumentType.NATIONAL_ID, localFileStorage_1.UserType.PARTICIPANT);
                ...
                if (files?.national_id_back?.[0]) {
                    ...
                    national_id_back_url = await (0, localFileStorage_1.saveToLocalStorage)(backFile.buffer, backFile.originalname, localFileStorage_1.DocumentType.NATIONAL_ID, localFileStorage_1.UserType.PARTICIPANT);
     ...
     */
    async getBookingParticipants(req, res) {
        try {
            const { bookingId } = req.params;
            const booking = await data_source_1.AppDataSource.getRepository("Booking").findOne({
                where: { id: bookingId },
    ...
    async removeParticipant(req, res) {
        try {
            ...
            const participantRepo = data_source_1.AppDataSource.getRepository("BookingParticipant");
            // Find participant
...
exports.ParticipantRegistrationController = ParticipantRegistrationController;
//# sourceMappingURL=ParticipantRegistrationController.js.map