...
exports.TeamMemberSubscriptionController = void 0;
const TeamMemberSubscriptionService_1 = require("../services/TeamMemberSubscriptionService");
...
const teamMemberSubscriptionService = new TeamMemberSubscriptionService_1.TeamMemberSubscriptionService();
...
class TeamMemberSubscriptionController {
    ...
    static async subscribeTeamMemberToTeam(req, res) {
        try {
            ...
            const result = await teamMemberSubscriptionService.subscribeTeamMemberToTeam(teamMemberId, teamId, {
     ...
     * @access  Staff only
     */
    static async unsubscribeTeamMemberFromTeam(req, res) {
        try {
            const user = req.user;
            const teamMemberId = parseInt(req.params.teamMemberId);
            ...
            const result = await teamMemberSubscriptionService.unsubscribeTeamMemberFromTeam(teamMemberId, subscriptionId, user.staff_id);
            res.status(200).json({
                success: true,
                message: result.message,
            });
     ...
     * @access  Staff only
     */
    static async approveTeamMemberSubscription(req, res) {
        try {
            const user = req.user;
            const subscriptionId = parseInt(req.params.subscriptionId);
            if (!user || !user.staff_id) {
            ...
            const result = await teamMemberSubscriptionService.approveTeamMemberSubscription(subscriptionId, user.staff_id);
            res.status(200).json({
                success: true,
                message: result.message,
                data: result.data,
     ...
     * @access  Staff only
     */
    static async rejectTeamMemberSubscription(req, res) {
        try {
            const user = req.user;
            const subscriptionId = parseInt(req.params.subscriptionId);
            if (!user || !user.staff_id) {
            ...
            const result = await teamMemberSubscriptionService.rejectTeamMemberSubscription(subscriptionId, user.staff_id);
            res.status(200).json({
                success: true,
                message: result.message,
            });
     ...
     * @access  Public
     */
    static async getTeamMemberSubscriptions(req, res) {
        try {
            const teamMemberId = parseInt(req.params.teamMemberId);
            if (isNaN(teamMemberId)) {
                res.status(400).json({
     ...
     * @access  Public
     */
    static async checkTeamMemberSubscription(req, res) {
        try {
            const teamMemberId = parseInt(req.params.teamMemberId);
            const teamId = req.params.teamId;
     ...
     * @access  Public
     */
    static async getTeamMemberSubscriptionCount(req, res) {
        try {
            const teamMemberId = parseInt(req.params.teamMemberId);
            if (isNaN(teamMemberId)) {
                res.status(400).json({
     ...
     */
    static async confirmTeamMemberSubscriptionPayment(req, res) {
        try {
            const user = req.user;
            ...
            const subscriptionRepo = data_source_1.AppDataSource.getRepository(TeamMemberTeam_1.TeamMemberTeam);
     ...
     */
    static async cancelTeamMemberSubscription(req, res) {
        try {
            const user = req.user;
            const subscriptionId = parseInt(req.params.subscriptionId, 10);
            const body = req.body;
            ...
            const isPendingPaymentDraft = normalizedStatus === 'pending' &&
                normalizedSubscriptionStatus === 'pending_payment';
            const isApprovedLike = normalizedStatus === 'approved' ||
            ...
            if (isPendingPaymentDraft && subscription.payment_reference) {
                try {
                    await paymentService.cancelPayment(subscription.payment_reference, reason || 'Subscription cancelled before payment was completed');
                }
                catch (paymentError) {
     ...
     */
    static async teamMemberSelfSubscribe(req, res) {
        try {
            const user = req.user;
            const body = req.body;
            ...
            const subscriptionRepository = data_source_1.AppDataSource.getRepository(TeamMemberTeam_1.TeamMemberTeam);
            ...
            const teamMember = await data_source_1.AppDataSource.getRepository(TeamMember_1.TeamMember).findOne({
            ...
            const existingSportSubscriptions = await data_source_1.AppDataSource.query(`
            ...
            `, [teamMemberId]);
            const existingSportIds = new Set(existingSportSubscriptions
                .map((row) => Number(row.sport_id))
                .filter((sportId) => Number.isFinite(sportId) && sportId > 0));
...
exports.TeamMemberSubscriptionController = TeamMemberSubscriptionController;
//# sourceMappingURL=TeamMemberSubscriptionController.js.map