...
const PrivilegeCalculationService_1 = __importDefault(require("./PrivilegeCalculationService"));
/**
...
class StaffService {
    constructor() {
        ...
        this.staffTypeRepository = data_source_1.AppDataSource.getRepository(StaffType_1.StaffType);
        this.privilegeRepository = data_source_1.AppDataSource.getRepository(Privilege_1.Privilege);
        this.staffPackageRepository = data_source_1.AppDataSource.getRepository(StaffPackage_1.StaffPackage);
        this.staffPrivilegeOverrideRepository = data_source_1.AppDataSource.getRepository(StaffPrivilegeOverride_1.StaffPrivilegeOverride);
    ...
    async getAllPrivilegePackages() {
        return await data_source_1.AppDataSource.query(`SELECT id, code, name_en, name_ar, description_en, description_ar, is_active, created_at, updated_at
       FROM packages
       WHERE is_active = true
       ORDER BY name_en`);
    }
     ...
     */
    async getPackagePrivileges(packageId) {
        return await data_source_1.AppDataSource.query(`
    ...
    async registerStaff(staffData) {
        ...
        const accountStatus = staffData.created_by_staff_type_id === 1 || staffData.created_by_staff_type_id === 2
            ? 'active'
            : 'pending';
        // Create account
     ...
     */
    async assignPackages(staffId, packageIds, assignedById) {
        ...
        for (const packageId of packageIds) {
            const pkg = await data_source_1.AppDataSource.query(`SELECT id FROM packages WHERE id = $1 AND is_active = true`, [packageId]);
            if (!pkg || pkg.length === 0) {
     ...
     */
    async grantPrivilege(staffId, privilegeId, assignedById, reason) {
        const staff = await this.staffRepository.findOne({ where: { id: staffId } });
        if (!staff) {
            throw new Error('Staff member not found');
        }
     ...
     */
    async revokePrivilege(staffId, privilegeId, assignedById, reason) {
        const staff = await this.staffRepository.findOne({ where: { id: staffId } });
        if (!staff) {
            throw new Error('Staff member not found');
        }
        ...
        const finalPrivileges = await PrivilegeCalculationService_1.default.calculateFinalPrivileges(staffId);
        const privilegeInfo = finalPrivileges.find((p) => p.id === privilegeId);
        if (!privilegeInfo) {
        ...
        const source = privilegeInfo.source;
        ...
        console.log(`[PRIVILEGE_REVOKED] Staff ${staffId}: Revoked ${privilege.code} (source: ${source}) by user ${assignedById}`);
        return {
            success: true,
     ...
     */
    async getStaffPrivileges(staffId) {
     ...
     */
    async hasPrivilege(staffId, privilegeCode) {
        const result = await data_source_1.AppDataSource.query(`
      SELECT EXISTS (
     ...
     */
    async getAllStaff(page = 1, limit = 10, role) {
        const whereClause = {};
        if (role) {
     ...
     */
    async deactivateStaff(staffId, deactivatedById) {
        const staff = await this.staffRepository.findOne({ where: { id: staffId } });
        if (!staff) {
            throw new Error('Staff member not found');
        }
    ...
    async getStaffActivityLogs(_staffId, _limit = 50) {
     ...
     */
    async getFinalPrivileges(staffId) {
        const staff = await this.staffRepository.findOne({ where: { id: staffId } });
        if (!staff) {
            throw new Error('Staff member not found');
        }
        return await PrivilegeCalculationService_1.default.calculateFinalPrivileges(staffId);
    }
    /**
     ...
     */
    async getFinalPrivilegeCodes(staffId) {
        const staff = await this.staffRepository.findOne({ where: { id: staffId } });
        if (!staff) {
            throw new Error('Staff member not found');
        }
        return await PrivilegeCalculationService_1.default.calculateFinalPrivilegeCodes(staffId);
    }
    /**
     ...
     */
    async staffHasPrivilege(staffId, privilegeCode) {
        return await PrivilegeCalculationService_1.default.hasPrivilege(staffId, privilegeCode);
    }
    /**
     ...
     */
    async staffHasAnyPrivilege(staffId, privilegeCodes) {
        return await PrivilegeCalculationService_1.default.hasAnyPrivilege(staffId, privilegeCodes);
    }
    /**
     ...
     */
    async staffHasAllPrivileges(staffId, privilegeCodes) {
        return await PrivilegeCalculationService_1.default.hasAllPrivileges(staffId, privilegeCodes);
    }
    /**
     ...
     */
    async getPrivilegeStats(staffId) {
        const staff = await this.staffRepository.findOne({ where: { id: staffId } });
        if (!staff) {
            throw new Error('Staff member not found');
        }
        return await PrivilegeCalculationService_1.default.getPrivilegeStats(staffId);
    }
    /**
     ...
     */
    async getPrivilegeBreakdown(staffId) {
        const staff = await this.staffRepository.findOne({ where: { id: staffId } });
        if (!staff) {
            throw new Error('Staff member not found');
        }
        // Get package privileges
        const packagePrivileges = await data_source_1.AppDataSource.query(`
      ...
      `, [staffId]);
        // Get assigned packages
        const assignedPackages = await data_source_1.AppDataSource.query(`
      ...
      `, [staffId]);
        ...
        const grantedOverrides = await data_source_1.AppDataSource.query(`
      ...
      `, [staffId]);
        ...
        const revokedOverrides = await data_source_1.AppDataSource.query(`
      SELECT p.id, p.code, p.name_en, p.name_ar, p.module, spo.assigned_at
      FROM staff_privileges_override spo
      JOIN privileges p ON spo.privilege_id = p.id
      ...
      `, [staffId]);
        ...
        return {
            staff_id: staffId,
            assigned_packages: assignedPackages.map((p) => ({
            ...
            privileges_from_packages: packagePrivileges.map((p) => ({
                privilege_id: p.id,
                code: p.code,
                name_en: p.name_en,
                name_ar: p.name_ar,
            ...
            individually_granted: grantedOverrides.map((p) => ({
                privilege_id: p.id,
                code: p.code,
                name_en: p.name_en,
                name_ar: p.name_ar,
            ...
            individually_revoked: revokedOverrides.map((p) => ({
                privilege_id: p.id,
                code: p.code,
                name_en: p.name_en,
                name_ar: p.name_ar,
            ...
            final_computed_privileges: finalPrivileges.map((p) => ({
                privilege_id: p.id,
                code: p.code,
                name_en: p.name_en,
                name_ar: p.name_ar,
    ...
    async createPrivilegePackage(packageData) {
        try {
            ...
            const existingPackage = await data_source_1.AppDataSource.query(`SELECT id FROM packages WHERE code = $1`, [packageData.code]);
            ...
            if (packageData.privilege_ids && packageData.privilege_ids.length > 0) {
                const validPrivileges = await data_source_1.AppDataSource.query(`SELECT id FROM privileges WHERE id = ANY($1) AND is_active = true`, [packageData.privilege_ids]);
            ...
            // Insert package
            const result = await data_source_1.AppDataSource.query(`INSERT INTO packages (code, name_en, name_ar, description_en, description_ar, is_active, created_at, updated_at)
         ...
         RETURNING id, code, name_en, name_ar, description_en, description_ar, is_active, created_at, updated_at`, [
            ...
            if (packageData.privilege_ids && packageData.privilege_ids.length > 0) {
                for (const privilegeId of packageData.privilege_ids) {
                    await data_source_1.AppDataSource.query(`INSERT INTO privileges_packages (package_id, privilege_id) VALUES ($1, $2)`, [packageId, privilegeId]);
                }
            }
            return {
                success: true,
     ...
     */
    async getPrivilegePackageById(packageId) {
        try {
            const packageData = await data_source_1.AppDataSource.query(`SELECT id, code, name_en, name_ar, description_en, description_ar, is_active, created_at, updated_at
         FROM packages
         WHERE id = $1`, [packageId]);
            ...
            const privileges = await data_source_1.AppDataSource.query(`SELECT p.id, p.code, p.name_en, p.name_ar, p.module, p.description_en, p.description_ar, p.is_active
         FROM privileges p
         JOIN privileges_packages pp ON p.id = pp.privilege_id
         WHERE pp.package_id = $1
         ORDER BY p.module, p.name_en`, [packageId]);
            return {
                success: true,
                data: {
                    ...pkg,
     ...
     */
    async updatePrivilegePackage(packageId, updateData) {
        try {
            // Check package exists
            const existingPackage = await data_source_1.AppDataSource.query(`SELECT id FROM packages WHERE id = $1`, [packageId]);
            if (existingPackage.length === 0) {
                throw new Error(`Package with ID ${packageId} not found`);
            }
            // If changing code, verify new code is unique
            if (updateData.code) {
                const codeExists = await data_source_1.AppDataSource.query(`SELECT id FROM packages WHERE code = $1 AND id != $2`, [updateData.code, packageId]);
            ...
            if (updateData.privilege_ids && updateData.privilege_ids.length > 0) {
                const validPrivileges = await data_source_1.AppDataSource.query(`SELECT id FROM privileges WHERE id = ANY($1) AND is_active = true`, [updateData.privilege_ids]);
            ...
            const result = await data_source_1.AppDataSource.query(updateQuery, values);
            // Update privileges if provided
            if (updateData.privilege_ids !== undefined) {
                // Remove existing privilege associations
                await data_source_1.AppDataSource.query(`DELETE FROM privileges_packages WHERE package_id = $1`, [packageId]);
     ...
     */
    async deletePrivilegePackage(packageId) {
        try {
            // Check package exists
            const existingPackage = await data_source_1.AppDataSource.query(`SELECT id, code FROM packages WHERE id = $1`, [packageId]);
            if (existingPackage.length === 0) {
                throw new Error(`Package with ID ${packageId} not found`);
            }
            const pkg = existingPackage[0];
            // Check if package is assigned to any staff
            const assignedStaff = await data_source_1.AppDataSource.query(`SELECT COUNT(*) as count FROM staff_packages WHERE package_id = $1`, [packageId]);
            if (assignedStaff[0].count > 0) {
                throw new Error(`Cannot delete package "${pkg.code}" because it is assigned to ${assignedStaff[0].count} staff members. Unassign it first or set is_active to false.`);
            }
            // Delete privilege associations
            await data_source_1.AppDataSource.query(`DELETE FROM privileges_packages WHERE package_id = $1`, [packageId]);
            // Delete package
            await data_source_1.AppDataSource.query(`DELETE FROM packages WHERE id = $1`, [packageId]);
            return {
                success: true,
     ...
     */
    async updatePackagePrivileges(packageId, privilegeIds) {
        try {
            // Check package exists
            const existingPackage = await data_source_1.AppDataSource.query(`SELECT id, code FROM packages WHERE id = $1`, [packageId]);
            if (existingPackage.length === 0) {
            ...
            if (privilegeIds.length > 0) {
                const validPrivileges = await data_source_1.AppDataSource.query(`SELECT id FROM privileges WHERE id = ANY($1) AND is_active = true`, [privilegeIds]);
