...
class FieldService {
    constructor() {
        ...
        this.operatingHoursRepo = data_source_1.AppDataSource.getRepository(FieldOperatingHours_1.FieldOperatingHours);
        this.sportRepo = data_source_1.AppDataSource.getRepository(Sport_1.Sport);
        this.branchRepo = data_source_1.AppDataSource.getRepository(Branch_1.Branch);
    }
    /**
     ...
     */
    async addOperatingHours(fieldId, hours) {
        const field = await this.fieldRepo.findOne({ where: { id: fieldId } });
        if (!field) {
            throw new Error(`Field with ID ${fieldId} not found`);
        }
        const operatingHours = hours.map(hour => this.operatingHoursRepo.create({
            field_id: fieldId,
     ...
     */
    async updateOperatingHours(fieldId, hours) {
        const field = await this.fieldRepo.findOne({ where: { id: fieldId } });
        if (!field) {
            throw new Error(`Field with ID ${fieldId} not found`);
        }
     ...
     */
    async getOperatingHours(fieldId) {
        const field = await this.fieldRepo.findOne({ where: { id: fieldId } });
        if (!field) {
            throw new Error(`Field with ID ${fieldId} not found`);
        }
     ...
     */
    async deleteOperatingHours(fieldId, dayOfWeek) {
     ...
     */
    async getAvailableFields(sportId) {
     ...
     */
    async getFieldsByBranch(branchId) {
        return await this.fieldRepo.find({
     ...
     */
    async checkAvailability(fieldId, dayOfWeek, time) {
     ...
     */
    async updateBookingSettings(fieldId, settings) {
        const field = await this.fieldRepo.findOne({ where: { id: fieldId } });
        if (!field) {
            throw new Error(`Field with ID ${fieldId} not found`);
        }
     ...
     */
    async getBookableFields(sportId) {
        const query = this.fieldRepo
            .createQueryBuilder('field')
            .leftJoinAndSelect('field.sport', 'sport')
            .leftJoinAndSelect('field.branch', 'branch')
     ...
     */
    async getBookableFieldsBySport() {
