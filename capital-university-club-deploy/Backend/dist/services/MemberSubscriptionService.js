...
class MemberSubscriptionService {
     ...
     */
    async subscribeMemberToSport(memberId, sportId, staffId) {
     ...
     */
    async unsubscribeMemberFromSport(memberId, sportId, staffId) {
        // Validate member exists
        const member = await this.memberRepository.findOne({
            where: { id: memberId },
        });
        ...
        const subscription = await data_source_1.AppDataSource.query(`
            SELECT * FROM member_sport_subscriptions 
            WHERE member_id = $1 AND sport_id = $2
        `, [memberId, sportId]);
     ...
     */
    async isMemberSubscribed(memberId, sportId) {
        const result = await data_source_1.AppDataSource.query(`
     ...
     */
    async getSportSubscriptionCount(sportId) {
        const result = await data_source_1.AppDataSource.query(`
            SELECT COUNT(*) FROM member_sport_subscriptions 
...
//# sourceMappingURL=MemberSubscriptionService.js.map